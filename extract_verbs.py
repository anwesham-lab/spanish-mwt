...
with open('spanish_verbs.mwt', mode='wt', encoding='utf-8') as file:
    file.write('\n\n'.join(mwt_strings))
...
# #key = r'([\d]+[\-][\d]+\t[a-zA-Z]*(?:\t_){8}\n)'
# term = r'[\d]+[\-][\d]+\t([a-zA-Z]*)(?:\t_){8}\n'
# start = r'([\d]+)[\-][\d]+\t[a-zA-Z]*(?:\t_){8}\n'
# end = r'[\d]+[\-]([\d]+)\t[a-zA-Z]*(?:\t_){8}\n'
# #mwt_keys = re.findall(key, train) + re.findall(key, dev) + re.findall(key, test)
# mwt_terms = re.findall(term, train) + re.findall(term, dev) + re.findall(term, test)
# mwt_starts = re.findall(start, train) + re.findall(start, dev) + re.findall(start, test)
# mwt_ends = re.findall(end, train) + re.findall(end, dev) + re.findall(end, test)
# for i, j in enumerate(mwt_terms):
#     if j == 'NUM':
#         del mwt_terms[i], mwt_starts[i], mwt_ends[i]#, mwt_keys[i]
#
# mwt_tokens = np.unique(mwt_terms)
# indexes = []
# use_indexes = []
# for token in mwt_tokens:
#     index = [i for i, term in enumerate(mwt_terms) if token == term]
#     indexes += [index]
#     use_indexes += [index[0]]
# print(mwt_tokens, '\n\n')
# print(indexes, '\n\n')
# print(use_indexes)
"""
for i, key in enumerate(mwt_text_list):
    mwt_pair_list.append([key, mwt_list[i]])
mwt_array = np.array(mwt_pair_list)
_,idx = np.unique(mwt_array[:,0],return_index=True)
out = mwt_array[idx]
mwt_texts = np.unique(mwt_text_list)
#print(mwt_texts)
print (mwt_array)
"""
