...
with open("imperatives.csv", 'r') as imperative_list:
    ...
    imperative_conjugates = csv.readlines(imperative_list)
...
for row in imperative_conjugates:
    ...
    for j in range(3):
        ...
        if re.search('rse', row[imperative_titles.index('infinitive')]) is not None:
            reflexive_verb = row[imperative_titles.index('infinitive')]
            infinitive_verb = reflexive_verb[:-2]
            mwt_token = row[imperative_titles.index(form)]
            if cap == 1:
            ...
            infinitive_verb = row[imperative_titles.index('infinitive')]
            conj_verb = row[imperative_titles.index(form)]
            syllables = 0
            i = len(conj_verb) - 1
            while i >= 0:
